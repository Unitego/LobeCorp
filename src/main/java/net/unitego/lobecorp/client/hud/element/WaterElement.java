    ...
    public void draw(GuiGraphics guiGraphics, float partialTick, int guiWidth, int guiHeight) {
        ...
        for (int water = 0; water < 10; water++) {
            ...
            if (minecraft.player.hasEffect(MobEffects.HUNGER)) {
                emptySprite = HUDResource.WATER_EMPTY_THIRST_SPRITE;
                halfSprite = HUDResource.WATER_HALF_THIRST_SPRITE;
                fullSprite = HUDResource.WATER_FULL_THIRST_SPRITE;
            } else {
                emptySprite = HUDResource.WATER_EMPTY_SPRITE;
                halfSprite = HUDResource.WATER_HALF_SPRITE;
                fullSprite = HUDResource.WATER_FULL_SPRITE;
            }

            if (waterData.getHydrationLevel() <= 0.0F && minecraft.gui.getGuiTicks() % (waterLevel * 3 + 1) == 0) {
        ...
        for (int hydration = 0; hydration < hydrationCount; hydration++) {
            ...
            if (value > 0 && value < 0.33F) {
                guiGraphics.blitSprite(HUDResource.HYDRATION_CRIT_SPRITE, posX, posY, size, size);
            } else if (value >= 0.33F && value < 0.66F) {
                guiGraphics.blitSprite(HUDResource.HYDRATION_LOW_SPRITE, posX, posY, size, size);
            } else if (value >= 0.66F && value < 1.0F) {
                guiGraphics.blitSprite(HUDResource.HYDRATION_STAB_SPRITE, posX, posY, size, size);
            } else if (value >= 1.0F) {
                guiGraphics.blitSprite(HUDResource.HYDRATION_FULL_SPRITE, posX, posY, size, size);
            }
        }
    }
}
