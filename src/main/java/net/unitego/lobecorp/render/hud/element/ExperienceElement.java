...
public class ExperienceElement extends BaseElement {
    ...
    public void draw(GuiGraphics guiGraphics, float partialTick, int guiWidth, int guiHeight) {
        ...
        //渲染经验条背景
        drawRect(guiGraphics, x, y, guiWidth, barHeight, ElementResource.BG1);
        drawRect(guiGraphics, x + gap, y + gap, guiWidth - gap * 2, barHeight - gap * 3, ElementResource.BG2);
        //渲染经验条
        drawRect(guiGraphics, x + gap, y + gap, (float) (experience * ratio), barHeight - gap * 3, ElementResource.EXPERIENCE_BAR);
        //渲染经验等级背景
        drawRect(guiGraphics, x + gap, y - txtHeight, expLevelOffset, txtHeight, ElementResource.BG1);
        //渲染经验值比例背景
        drawRect(guiGraphics, guiWidth - expRatioOffset, y - txtHeight, expRatioOffset, txtHeight, ElementResource.BG1);
        //渲染经验等级字体
        guiGraphics.pose().scale(0.5f, 0.5f, 0.5f);
        guiGraphics.drawCenteredString(minecraft.font, expLevelString, (x + gap) * 2 + expLevelOffset, (y - txtHeight) * 2 + heightOffset, ElementResource.EXPERIENCE_BAR);
        guiGraphics.pose().scale(2.0f, 2.0f, 2.0f);
        //渲染经验值比例字体
        guiGraphics.pose().scale(0.5f, 0.5f, 0.5f);
        guiGraphics.drawCenteredString(minecraft.font, expRatioString, (guiWidth - expRatioOffset) * 2 + expRatioOffset, (y - txtHeight) * 2 + heightOffset, ElementResource.EXPERIENCE_BAR);
        guiGraphics.pose().scale(2.0f, 2.0f, 2.0f);
    }
}
