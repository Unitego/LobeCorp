    ...
    public void draw(GuiGraphics guiGraphics, float partialTick, int guiWidth, int guiHeight) {
        if (Objects.requireNonNull(minecraft.gameMode).getPlayerMode() == GameType.SPECTATOR) {
            ...
            if (player.isCreative()) {
                drawRect(guiGraphics, x, y + height, width, height - size, ElementResource.BG1);
            }
            //渲染九个快捷栏栏位
            for (int slot = 0; slot < 9; slot++) {
                drawRect(guiGraphics, x + gap + slot * (size + gap), y + gap, size, size, ElementResource.BG3);
            }
            //渲染当前快捷栏栏位
            drawRect(guiGraphics, x + gap + player.getInventory().selected * (size + gap), y + gap, size, size, ElementResource.BG4);
            //副手不为空时渲染副手栏
            if (!offhandItem.isEmpty()) {
                if (mainArm == HumanoidArm.RIGHT) {
                    drawRect(guiGraphics, x - gap * 3 - size, y, gap * 2 + size, gap + size, ElementResource.BG1);
                    drawRect(guiGraphics, x - gap * 2 - size, y + gap, size, size, ElementResource.BG4);
                    if (player.isCreative()) {
                        drawRect(guiGraphics, x - gap * 3 - size, y + height, gap * 2 + size, gap, ElementResource.BG1);
                    }
                } else if (mainArm == HumanoidArm.LEFT) {
                    drawRect(guiGraphics, x + width + gap, y, gap * 2 + size, gap + size, ElementResource.BG1);
                    drawRect(guiGraphics, x + width + gap * 2, y + gap, size, size, ElementResource.BG4);
                    if (player.isCreative()) {
                        drawRect(guiGraphics, x + width + gap, y + height, gap * 2 + size, gap, ElementResource.BG1);
                    }
                }
            }
