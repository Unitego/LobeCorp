...
public class MountHealthElement extends BaseElement {
    ...
    public void draw(GuiGraphics guiGraphics, float partialTick, int guiWidth, int guiHeight) {
        ...
        if (vehicle instanceof LivingEntity mount) {
            ...
            //渲染坐骑生命条背景
            drawTrap(x, y, widthTop, widthBottom, barHeight, ElementResource.BG1);
            drawTrap(x + gap, y, widthTop - gap * 2, widthBottom - gap, barHeight - gap, ElementResource.BG2);
            //渲染坐骑生命条
            drawTrap(x + gap, y, (widthTop - gap * 2) * ratio, (widthBottom - gap) * ratio, barHeight - gap, ElementResource.NORMAL_HEALTH_BAR);

            //渲染坐骑生命值背景
            drawRect(guiGraphics, x + gap, y + barHeight, mountHealthOffset, txtHeight, ElementResource.BG1);
            //渲染坐骑生命值字体
            guiGraphics.pose().scale(0.5f, 0.5f, 0.5f);
            guiGraphics.drawCenteredString(minecraft.font, mountHealthString, (x + gap) * 2 + mountHealthOffset, (y + barHeight) * 2 + heightOffset, ElementResource.TXT);
