...
public abstract class BaseElement {
     ...
     */
    public static void drawRect(GuiGraphics guiGraphics, int posX, int posY, float width, float height, int color) {
        // 跳过无效颜色
        if (color == -1) return;
     ...
     * @param color       ARGB格式颜色值（-1表示跳过绘制）
     */
    public static void drawTrap(int posX, int posY, double widthTop, double widthBottom, int height, int color) {
        if (color == -1) return;
        float alpha;
        if (color >= 0 && color <= 0xffffff) alpha = 1.0f;
