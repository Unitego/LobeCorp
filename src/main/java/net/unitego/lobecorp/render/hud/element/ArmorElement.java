...
public class ArmorElement extends BaseElement {
    ...
    public void draw(GuiGraphics guiGraphics, float partialTick, int guiWidth, int guiHeight) {
        ...
        if (armor > 0) {
            ...
            //渲染护甲值图标
            guiGraphics.blitSprite(Gui.ARMOR_FULL_SPRITE, x + gap, y, size, size);
            //渲染护甲值字体
            guiGraphics.pose().scale(0.5f, 0.5f, 0.5f);
            guiGraphics.drawCenteredString(minecraft.font, armorString, (x + gap + size) * 2 + armorOffset, y * 2 + heightOffset, ElementResource.TXT);
            guiGraphics.pose().scale(2.0f, 2.0f, 2.0f);
        }
        if (armorToughness > 0) {
            //渲染盔甲韧性背景
            drawRect(guiGraphics, x, y + height + gap, width, height, ElementResource.BG1);
            //渲染盔甲韧性图标
            guiGraphics.blitSprite(ElementResource.ARMOR_TOUGHNESS_FULL_SPRITE, x + gap, y + height + gap, size, size);
            //渲染盔甲韧性字体
            guiGraphics.pose().scale(0.5f, 0.5f, 0.5f);
            guiGraphics.drawCenteredString(minecraft.font, armorToughnessString, (x + gap + size) * 2 + armorToughnessOffset, (y + height + gap) * 2 + heightOffset, ElementResource.TXT);
            guiGraphics.pose().scale(2.0f, 2.0f, 2.0f);
        }
    }
}
